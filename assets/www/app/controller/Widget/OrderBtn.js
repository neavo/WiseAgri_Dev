				...
				if (Data[k]) {
					...
					for (var Value in DB.OrderApp) {
						if (Data[k]["AppId"] == DB.OrderApp[Value]["AppId"]) {
							Ordered = true;
							ItemHtml = "<img class = OrderViewIcon src = " + Data[k]["AppIconUrl"] + " />"
								 + "<img style = \""
								 + " width : " + DB.ScreenWidth * 0.3 + "px;"
								 + " height : " + DB.ScreenWidth * 0.3 + "px;"
								 + " position : relative;"
								 + " top : -" + DB.ScreenWidth * 0.315 + "px; \""
								 + " src = resources/image/Checked.png />";
						};
					};
